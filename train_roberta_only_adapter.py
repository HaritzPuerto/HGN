...
from transformers import get_linear_schedule_with_warmup, RobertaModelAdapter
...
def get_training_params(encoder, pred_layer, print_stats=False):
    params = []
    params_name = []
    params_name_frozen = []
    ...
    training_params = ['adapter', 'prediction_layer']
    ...
    # add pred_layer params
    for n, p in pred_layer.named_parameters():
        num_training_params += p.numel()
        params.append(p)
        params_name.append(n)
        dict_params['prediction_layer'] += p.numel()
...
def get_optimizer(model, pred_layer, args, learning_rate, remove_pooler=False):
    """
    ...
    """
    num_training_params = 0
    params_name, params = get_training_params(model, pred_layer, print_stats=True)
...
encoder = RobertaModelAdapter.from_pretrained(args.encoder_name_or_path)
