...
class GraphBlock(nn.Module):
    ...
    def forward(self, batch, input_state, query_vec):
        ...
        para_start_output = torch.bmm(para_start_mapping, input_state)   # N x max_para x d
        para_end_output = torch.bmm(para_end_mapping, input_state)       # N x max_para x d
        para_state = torch.cat([para_start_output, para_end_output], dim=-1)  # N x max_para x 2d

        sent_start_output = torch.bmm(sent_start_mapping, input_state)   # N x max_sent x d
        sent_end_output = torch.bmm(sent_end_mapping, input_state)       # N x max_sent x d
        sent_state = torch.cat([sent_start_output, sent_end_output], dim=-1)  # N x max_sent x 2d

        ent_start_output = torch.bmm(ent_start_mapping, input_state)   # N x max_ent x d
        ent_end_output = torch.bmm(ent_end_mapping, input_state)       # N x max_ent x d
