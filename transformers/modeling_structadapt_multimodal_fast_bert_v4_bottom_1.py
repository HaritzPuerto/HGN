...
"""PyTorch BERT model. 
Same as version 2 but with a residual connection in each graph embedding
"""
...
class BiModalAdapter(nn.Module):
    ...
    def forward(self, hidden_states, batch, graph_emb_prev_layer):
        layer_output_text = self.adapter_text(hidden_states) # text adapter
        
        graph_out_dict = self.adapter_graph(hidden_states, batch, graph_emb_prev_layer) # graph adapter
...
class BertLayer(nn.Module):
    def __init__(self, config, hgn_config):
        ...
        self.adapter_bottom = Adapter(config_adapters)
    ...
    def forward(
        ...
        #### Top Adapter
        layer_output, graph_out_dict = self.adapter_bimodal(layer_output, batch, graph_emb_prev_layer)
...
class GraphBlock(nn.Module):
    ...
    def forward(self, batch, input_state, query_vec, graph_emb_prev_layer=None):
        ...
        if graph_emb_prev_layer is not None:
            graph_state = graph_state + graph_emb_prev_layer['graph_state']
