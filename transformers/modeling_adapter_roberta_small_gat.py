...
from .modeling_adapter_bert_small_gat import BertEmbeddings, BertLayerNorm, BertModel, BertPreTrainedModel, gelu
...
class AdapterGraphQA_small_gat(torch.nn.Module):
    def __init__(self, pretrained_weights, args):
        ...
        self.entity_mlp = OutputLayer(self.hidden_size, self.encoder.config, num_answer=1)
        # Span Prediction
    ...
    def forward(
            ...
            (transformer_output, graph_output) = self.encoder(
                batch,
                input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
            ...
            # return transformer_output, graph_output
            
            graph_state = graph_output['graph_state']
            ent_state = graph_state[:, 1+graph_output['max_para_num']+graph_output['max_sent_num']:, :]
            
            gat_logit = self.sent_mlp(graph_output['graph_state'][:, :1+graph_output['max_para_num']+graph_output['max_sent_num'], :]) # N x max_sent x 1
            para_logit = gat_logit[:, 1:1+graph_output['max_para_num'], :].contiguous()
            sent_logit = gat_logit[:, 1+graph_output['max_para_num']:, :].contiguous()

            ent_logit = self.entity_mlp(ent_state).view(graph_output['N'], -1)
