...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # text modality
        text_self_attention_outputs = self.attention(text_hidden_states, attention_mask, head_mask)
        text_attention_output = text_self_attention_outputs[0]
        text_outputs = text_self_attention_outputs[1:]  # add self attentions if we output attention weights
        
        # graph modality
        graph_self_attention_outputs = self.attention(graph_hidden_states, attention_mask, head_mask)
        graph_attention_output = graph_self_attention_outputs[0]
        graph_outputs = graph_self_attention_outputs[1:]  # add self attentions if we output attention weights

        if self.is_decoder and encoder_hidden_states is not None:
            # text modality
            text_cross_attention_outputs = self.crossattention(
                text_attention_output, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask
            )
            text_attention_output = text_cross_attention_outputs[0]
            text_outputs = text_outputs + text_cross_attention_outputs[1:]  # add cross attentions if we output attention weights
            
            # graph modality
            graph_cross_attention_outputs = self.crossattention(
                graph_attention_output, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask
            )
            graph_attention_output = graph_cross_attention_outputs[0]
            graph_outputs = graph_outputs + graph_cross_attention_outputs[1:]  # add cross attentions if we output attention weights

        #### Bottom Adapter
        attention_output_text = self.adapter_text_bottom(text_attention_output)
        attention_output_graph = self.adapter_graph_bottom(graph_attention_output)
        ...
        text_outputs = (layer_output,) + text_outputs
        graph_outputs = (layer_output_graph,) + graph_outputs
        return (text_outputs, graph_outputs, graph_out_dict)
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        graph_hidden_states = hidden_states # initialize graph hidden states with the embeddings
        for i, layer_module in enumerate(self.layer):
            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)

            layer_outputs, graph_outputs, graph_dict = layer_module(
                hidden_states, graph_hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, batch=batch,
            )
            hidden_states = layer_outputs[0]
            graph_hidden_states = graph_outputs[0]
        ...
        return outputs, graph_hidden_states, graph_dict  # last-layer hidden state, (all hidden states), (all attentions)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs, graph_outputs, graph_dict = self.encoder(
            embedding_output,
        ...
        return outputs, graph_outputs, graph_dict  # sequence_output, pooled_output, (hidden_states), (attentions)
