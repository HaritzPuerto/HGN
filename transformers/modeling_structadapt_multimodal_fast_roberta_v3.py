...
This StructAdapt v3 concatenates the node embeddings across layers to do node classification.
...
from .modeling_structadapt_multimodal_fast_bert_v3 import BertEmbeddings, BertLayerNorm, BertModel, BertPreTrainedModel, gelu
...
class MultiModalStructAdaptFastRoberta_v3(nn.Module):
    def __init__(self, config):
        super(MultiModalStructAdaptFastRoberta_v3, self).__init__()     
        ...
        self.pred_layer = PredictionLayer(config, self.encoder.config.num_hidden_layers)
    ...
    def forward(
            ...
            input_state, all_graph_emb = self.encoder(
            ...
            return self.pred_layer(batch, input_state[0], all_graph_emb, packing_mask=query_mapping, return_yp=return_yp)
...
class PredictionLayer(nn.Module):
    ...
    """
    def __init__(self, config, encoder_num_layers):
        ...
        # Node Classification
        self.proj_linear = nn.Linear(encoder_num_layers*config.adapter_size*2, config.hidden_dim*2)
    ...
    def forward(self, batch, context_input, all_graph_emb, packing_mask=None, return_yp=False):
        # Node Classification
        list_ent_states = []
        list_input_sent = []
        for graph_out in all_graph_emb:
            ent_state = graph_out['graph_state'][:, 1+graph_out['max_para_num']+graph_out['max_sent_num']:, :] # (batch_size, max_ent_num, adapter_size*2)
            input_sent = graph_out['graph_state'][:, :1+graph_out['max_para_num']+graph_out['max_sent_num'], :]
            
            list_ent_states.append(ent_state)
            list_input_sent.append(input_sent)

        ent_state_across_layers = self.proj_linear(torch.cat(list_ent_states, dim=-1))
        input_sent_across_layers = self.proj_linear(torch.cat(list_input_sent, dim=-1))

        gat_logit = self.sent_mlp(input_sent_across_layers) # N x max_sent x 2
        ...
        ent_prediction = self.entity_mlp(ent_state_across_layers).view(graph_out['N'], -1)
